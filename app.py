    ...
    if session['user_status'] == constants.ADMIN:
        try:
            ...
            with connection.cursor() as cursor:
                cursor.execute("SELECT name, mail, status, date, score FROM log_table ORDER BY id DESC")
                log_data = cursor.fetchall()
                cursor.execute("SELECT exception_data, exception_code, user_mail, user_status, exception_date \
                    FROM exception_table ORDER BY id DESC")
...
#CREATE TABLE log_table (ID INTEGER NOT NULL PRIMARY KEY GENERATED BY DEFAULT AS IDENTITY, name VARCHAR(50), mail VARCHAR(50),status VARCHAR(50), date VARCHAR(50) , score real );
