    ...
    if request.method == 'POST' and (session['user_status'] == ADMIN or session['user_status'] == COACH):
        ...
        if listSize == 'small':
            ...
            try:
                ...
                connection.autocommit = True
                with connection.cursor() as cursor:
                    cursor.execute("SELECT name_test, mail , reliability  , organizational ,  building_on_achievements , approved , country , clientoority , adoption_of_decisions , effective_communication ,  management , implementation_management FROM test_results ORDER BY mail")
                    testResults = cursor.fetchall()
            except Exception as _ex:
                print("[INFO] Erroe while working with PostgraseSQL", _ex)
                flash('Не удалось подключиться к базе данных. Попробуйте повторить попытку.')
